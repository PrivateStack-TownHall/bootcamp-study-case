...
describe('CartService', () => {
  ...
  describe('create', () => {
    it('throws NotFoundException when the product does not exist', async () => {
      ...
      await expect(
        service.create(1, { productId: 95, quantity: 1 } as any),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws BadRequestException when stock is insufficient', async () => {
      prisma.product.findUnique.mockResolvedValue({ id: 95, stock: 1 });

      await expect(
        service.create(1, { productId: 95, quantity: 5 } as any),
    ...
    it('increments quantity and returns "Cart updated successfully" when the item already exists in cart (no audit log on this path)', async () => {
      prisma.product.findUnique.mockResolvedValue({ id: 95, stock: 10 });
  ...
  describe('findAll', () => {
    it('returns totalItems, totalAmount and { data } with numeric prices', async () => {
      prisma.cartItem.findMany.mockResolvedValue([
        { id: 1, quantity: 2, product: { price: '2499000' } },
      ]);

      const result = await service.findAll(1);

      expect(result.totalItems).toBe(1);
      expect(result.totalAmount).toBe(4998000);
      expect(result.data[0].product.price).toBe(2499000);
    });
  });
  ...
  describe('findOne', () => {
    it('returns { data } for a cart item owned by the user', async () => {
      ...
      expect(result.data.product.price).toBe(2499000);
    });
  ...
  describe('update', () => {
    it('updates quantity, logs an audit entry, and returns { message, data }', async () => {
      ...
      // Regression guard: Quantum-Mart's cart update() logs an UPDATE_CART
      // audit entry, unlike Kings-Brew/Castle-Kitchen/Trade-Hub's update().
      expect(auditLogsService.create).toHaveBeenCalledWith(
