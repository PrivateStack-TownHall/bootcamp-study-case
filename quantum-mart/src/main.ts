...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Quantum Mart API')
    .setDescription(
      'General Marketplace Backend API — retail ordering platform ' +
        'covering auth, categories, inventory, cart, checkout, orders, ' +
        'payments, reviews, order status tracking, and audit logging.',
    )
    .setVersion('1.0')
    .addBearerAuth({
      type: 'http',
