...
describe('PaymentsService', () => {
  ...
  describe('create', () => {
    ...
    it('creates a payment and returns { message, data } with amount as a Number', async () => {
      ...
      expect(result.data.amount).toBe(4998000);
  ...
  describe('findAll', () => {
    it('returns { data } scoped to the user with amount as a Number', async () => {
      prisma.payment.findMany.mockResolvedValue([{ id: 1, amount: '4998000' }]);
      ...
      expect(prisma.payment.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            order: expect.objectContaining({ userId: 1 }),
          }),
        }),
      );
      expect(result.data[0].amount).toBe(4998000);
    });
  });

  describe('findOne', () => {
    it('returns { data } with amount as a Number', async () => {
      prisma.payment.findFirst.mockResolvedValue({ id: 1, amount: '4998000' });
