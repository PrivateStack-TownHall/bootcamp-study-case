...
describe('FavoritesService', () => {
  ...
  describe('create', () => {
    it('adds a favorite and returns { message, data }', async () => {
      ...
      const favorite = { id: 1, userId: 1, productId: 95 };
      prisma.favorite.create.mockResolvedValue(favorite);

      const result = await service.create(1, 95);
    ...
    it('throws BadRequestException when already favorited', async () => {
      ...
      await expect(service.create(1, 95)).rejects.toThrow(BadRequestException);
  ...
  describe('findAll', () => {
    it('returns { data } scoped to the current user', async () => {
      const favorites = [{ id: 1, userId: 1, productId: 95 }];
