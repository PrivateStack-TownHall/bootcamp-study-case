...
describe('ProductImagesService', () => {
  ...
  describe('create', () => {
    it('creates an image, defaults sortOrder to 0, returns { message, data }', async () => {
      const image = { id: 149, productId: 95, imageUrl: 'url', sortOrder: 0 };
      ...
      expect(prisma.productImage.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { productId: 95, imageUrl: 'url', sortOrder: 0 },
        }),
      );
  ...
  describe('findAll', () => {
    it('returns { data } ordered by sortOrder', async () => {
      const images = [{ id: 149, sortOrder: 1 }];
  ...
  describe('findOne', () => {
    it('returns { data } for an existing image', async () => {
      ...
      const result = await service.findOne(149);
  ...
  describe('update', () => {
    it('updates and returns { message, data }', async () => {
      prisma.productImage.findFirst.mockResolvedValue({ id: 149 });
      const updated = { id: 149, sortOrder: 2 };
      prisma.productImage.update.mockResolvedValue(updated);

      const result = await service.update(149, { sortOrder: 2 } as any);
  ...
  describe('remove', () => {
    it('deletes and returns only { message }', async () => {
      prisma.productImage.findFirst.mockResolvedValue({ id: 149 });
      prisma.productImage.delete.mockResolvedValue({ id: 149 });

      const result = await service.remove(149);
