...
describe('CategoriesService', () => {
  ...
  describe('create', () => {
    it('creates a category and wraps it in { message, data }', async () => {
      ...
      expect(prisma.category.create).toHaveBeenCalledWith({
        data: { appType: 'RESTAURANT', name: 'Steaks', description: 'desc' },
      });
  ...
  describe('findAll', () => {
    it('returns categories wrapped in { data }', async () => {
      const categories = [{ id: 1, name: 'Steaks' }];
  ...
  describe('findOne', () => {
    it('returns the category wrapped in { data }', async () => {
      const category = { id: 1, name: 'Steaks' };
  ...
  describe('update', () => {
    it('updates and returns { message, data }', async () => {
      prisma.category.findUnique.mockResolvedValue({ id: 1, name: 'Steaks' });
  ...
  describe('remove', () => {
    it('deletes and returns only { message }', async () => {
      ...
      expect(result).toEqual({ message: 'Menu category deleted successfully' });
    });
