...
describe('CartService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      // Note: Castle-Kitchen's cart.service.ts looks up the product with
      // findUnique (not findFirst) before adding to cart.
  ...
  describe('create', () => {
    it('throws NotFoundException when the menu item does not exist', async () => {
  ...
  describe('findAll', () => {
    it('returns totalItems, totalAmount and { data } with numeric prices', async () => {
      prisma.cartItem.findMany.mockResolvedValue([
        { id: 1, quantity: 2, product: { price: '150000' } },
      ]);
      ...
      expect(result.data[0].product.price).toBe(150000);
    });
  });
  ...
  describe('findOne', () => {
    it('returns { data } for a cart item owned by the user', async () => {
      ...
      expect(result.data.product.price).toBe(150000);
    });
