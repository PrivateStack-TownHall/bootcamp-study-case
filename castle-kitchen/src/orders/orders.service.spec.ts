...
describe('OrdersService', () => {
      ...
      prisma.cartItem.findMany.mockResolvedValue([
        {
          productId: 1,
          quantity: 5,
          product: { id: 1, name: 'Sirloin Steak', price: '150000', stock: 1 },
        },
      ]);
      ...
      prisma.cartItem.findMany.mockResolvedValue([
        {
          productId: 1,
          quantity: 2,
          product: { id: 1, name: 'Sirloin Steak', price: '150000', stock: 10 },
        },
      ]);

      const createdOrder = { id: 1, orderNumber: 'CK-1', totalAmount: 300000 };
  ...
  describe('findAll', () => {
    it('returns { data } with totalAmount converted to Number, scoped to the user', async () => {
      ...
      expect(result.data[0].totalAmount).toBe(300000);
    });
  });
  ...
  describe('updateStatus', () => {
    it('updates status, records history + audit log, and returns { message, data }', async () => {
      ...
      // silently dropping `data` even though Swagger documents it — and it
      // also used to filter by AppType.COFFEE instead of RESTAURANT, which
      // meant this call would always 404 for a real Castle-Kitchen order.
