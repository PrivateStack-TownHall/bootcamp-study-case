...
describe('PaymentsService', () => {
  ...
  describe('findAll', () => {
    it('returns { data } scoped to the user with amount as a Number', async () => {
      prisma.payment.findMany.mockResolvedValue([{ id: 1, amount: '300000' }]);
      ...
      expect(result.data[0].amount).toBe(300000);
    });
  });

  describe('findOne', () => {
    it('returns { data } with amount as a Number', async () => {
      prisma.payment.findFirst.mockResolvedValue({ id: 1, amount: '300000' });
