...
describe('ProductsService', () => {
  ...
  describe('create', () => {
    it('creates a menu item, converts Decimal price to Number, and wraps { message, data }', async () => {
  ...
  describe('findAll', () => {
    it('returns paginated { meta, data } with prices converted to Number', async () => {
      prisma.product.findMany.mockResolvedValue([{ id: 1, price: '150000' }]);
    ...
    it('applies search filter as case-insensitive contains', async () => {
      ...
      await service.findAll({ search: 'steak' } as any);
  ...
  describe('findOne', () => {
    it('returns { data } with price converted to Number', async () => {
      prisma.product.findUnique.mockResolvedValue({ id: 1, price: '150000' });

      const result = await service.findOne(1);

      expect(result).toEqual({ data: { id: 1, price: 150000 } });
    });
  ...
  describe('update', () => {
    it('updates and returns { message, data }', async () => {
      ...
      expect(result.data.price).toBe(180000);
    });

    it('throws NotFoundException before updating when menu item is missing', async () => {
      prisma.product.findUnique.mockResolvedValue(null);
  ...
  describe('remove', () => {
    it('deletes and returns only { message }', async () => {
      ...
      expect(result).toEqual({ message: 'Menu deleted successfully' });
    });

    it('throws NotFoundException before deleting when menu item is missing', async () => {
      prisma.product.findUnique.mockResolvedValue(null);
