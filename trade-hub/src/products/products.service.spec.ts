...
describe('ProductsService', () => {
  ...
  describe('create', () => {
    it('creates a product, converts Decimal price to Number, and wraps { message, data }', async () => {
      ...
      expect(result.message).toBe('Catalog item created successfully');
  ...
  describe('findAll', () => {
    it('returns paginated { meta, data } with prices converted to Number', async () => {
      prisma.product.findMany.mockResolvedValue([{ id: 1, price: '500000' }]);
      ...
      expect(result.data[0].price).toBe(500000);
    });
    ...
    it('applies search filter as case-insensitive contains', async () => {
      ...
      await service.findAll({ search: 'keyboard' } as any);
  ...
  describe('findOne', () => {
    it('returns { data } with price converted to Number', async () => {
      prisma.product.findUnique.mockResolvedValue({ id: 1, price: '500000' });

      const result = await service.findOne(1);

      expect(result).toEqual({ data: { id: 1, price: 500000 } });
    });
  ...
  describe('update', () => {
    it('updates and returns { message, data }', async () => {
      ...
      expect(result.message).toBe('Catalog item updated successfully');
      expect(result.data.price).toBe(600000);
    });
