...
describe('CartService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      // Note: unlike the sibling apps, Trade-Hub's cart.service.ts looks up
      // the product with findUnique (not findFirst) before adding to cart.
  ...
  describe('findAll', () => {
    it('returns totalItems, totalAmount and { data } with numeric prices', async () => {
      prisma.cartItem.findMany.mockResolvedValue([
        { id: 1, quantity: 2, product: { price: '500000' } },
      ]);
      ...
      expect(result.data[0].product.price).toBe(500000);
    });
  });
  ...
  describe('findOne', () => {
    it('returns { data } for a cart item owned by the user', async () => {
      ...
      expect(result.data.product.price).toBe(500000);
    });
